...
public interface IUsuarioServicesDAO {
    ...
    public UsuarioDTO update(int id, UsuarioDTO usuarioDTO); //PUT /players: modifica el nom del jugador/a.

    public void deleteJugada(int id); //DELETE /players/{id}/games: elimina les tirades del jugador/a.

    public JugadaEntity getJugada(int id); //POST /players/{id}/games/ : un jugador/a específic realitza una tirada dels daus.
    ...
    public List<JugadaEntity> getOne(int id); //GET /players/{id}/games: retorna el llistat de jugades per un jugador/a.
    ...
    boolean validationToken(int id, HttpServletRequest request); //método para validar al usuario mediante la comparación del id del token con el id del usuario.
}